...
import org.ccbcj.crypto.MnemonicCode;
...
public class WalletSettingsController {
    ...
    public void initialize(@Nullable KeyParameter aesKey) {
        DeterministicSeed seed = Main.ccbc.wallet().getKeyChainSeed();
        if (aesKey == null) {
            if (seed.isEncrypted()) {
            ...
            this.aesKey = aesKey;
            seed = seed.decrypt(checkNotNull(Main.ccbc.wallet().getKeyCrypter()), "", aesKey);
            // Now we can display the ccbc seed as appropriate.
            passwordButton.setText("Remove password");
        }

        // Set the date picker to show the birthday of this ccbc.
        ...
        // Don't let the user click restore if the words area contains the current ccbc words, or are an invalid set,
    ...
    public void restoreClicked(ActionEvent event) {
        // Don't allow a restore unless this ccbc is presently empty. We don't want to end up with two wallets, too
        // much complexity, even though WalletAppKit will keep the current one as a backup file in case of disaster.
        if (Main.ccbc.wallet().getBalance().value > 0) {
            informationalAlert("Wallet is not empty",
                    "You must empty this ccbc out before attempting to restore an older one, as mixing wallets " +
        ...
        if (aesKey != null) {
            ...
            informationalAlert("Wallet is encrypted",
                    "After restore, the ccbc will no longer be encrypted and you must set a new password.");
        }

        log.info("Attempting ccbc restore using seed '{}' from date {}", wordsArea.getText(), datePicker.getValue());
        informationalAlert("Wallet restore in progress",
                "Your ccbc will now be resynced from the Bitcoin network. This can take a long time for old wallets.");
        overlayUI.done();
        ...
        // Shut down ccbcj and restart it with the new seed.
        Main.ccbc.addListener(new Service.Listener() {
            @Override
            public void terminated(Service.State from) {
                Main.instance.setupWalletKit(seed);
                Main.ccbc.startAsync();
            }
        }, Platform::runLater);
        Main.ccbc.stopAsync();
    }
    ...
    public void passwordButtonClicked(ActionEvent event) {
        if (aesKey == null) {
            ...
            Main.ccbc.wallet().decrypt(aesKey);
